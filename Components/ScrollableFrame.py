...
class ScrollableFrame(tk.Frame):
    def __init__(self, container, *args, **kwargs):
        ...
        self.scrollableFrame = tk.Frame(canvas, bg="black")
...
class ScrollableFramePin(tk.Frame):
    ...
    def mainArea(self):
        return {
            "width": (
                self.canvases[0].winfo_width()
                + self.canvases[1].winfo_width()
            ),
            "height": (
                self.canvases[0].winfo_height()
                + self.canvases[2].winfo_height()
            )
        }
    ...
    def setTop(self, *args):
        ...
        area = self.mainArea()
        if (
            event.height == self.pinHeight
            or area["height"] < self.kwargs["height"] - 20
        ):
            return
        height = event.height
    ...
    def setSide(self, *args):
        ...
        area = self.mainArea()
        if (
            event.width == self.pinWidth
            or area["width"] < self.kwargs["width"] - 20
        ):
            return
        width = event.width
